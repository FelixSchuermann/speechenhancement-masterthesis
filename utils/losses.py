...
def pmsqe_log_mse_loss(I=8):
    def pmsqe_log_mse_loss_inner(y_true, y_pred):
        ...
        stft_true= 10.0**(0.5 * (y_true + log10(1)/10))
        stft_pred= 10.0**(0.5 * (y_pred + log10(1)/10))

        #stft_true = K.square(K.abs(tf.contrib.signal.stft(y_true,256,128,256,window_fn,pad_end=True)))
        #stft_pred = K.square(K.abs(tf.contrib.signal.stft(y_pred,256,128,256,window_fn,pad_end=True)))
        print("stft_true shape:   ", K.int_shape(stft_true))
        print("stft_pred shape:   ", K.int_shape(stft_pred))

        # Init PMSQE required constants
        pmsqe.init_constants(Fs=16000, Pow_factor=pmsqe.perceptual_constants.Pow_correc_factor_Hann, apply_SLL_equalization=True,
